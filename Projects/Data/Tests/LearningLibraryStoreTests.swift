...
final class LearningLibraryStoreTests: XCTestCase {
    func test_캐시가_없으면_구독해도_아무것도_방출하지_않는다() async {
        let store = LearningLibraryStore()
        let (stream, continuation) = AsyncStream<LearningLibrary>.makeStream()

        await store.register(id: UUID(), continuation: continuation)
        continuation.finish()

        var received: [LearningLibrary] = []
    ...
    func test_구독자_2명이_동일한_set을_모두_받는다() async {
        ...
        let (streamA, continuationA) = AsyncStream<LearningLibrary>.makeStream()
        let (streamB, continuationB) = AsyncStream<LearningLibrary>.makeStream()
        ...
        var receivedA: [LearningLibrary] = []
        for await value in streamA { receivedA.append(value) }
        var receivedB: [LearningLibrary] = []
...
private func makeLibrary(levelID: String) -> LearningLibrary {
    LearningLibrary(levels: [
        LevelSummary(
            id: levelID,
            level: 1,
            name: "Level 1",
